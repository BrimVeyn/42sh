...
/*   Updated: 2024/12/30 00:47:26 by bvan-pae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void builtin_alias(const SimpleCommandP *command, Vars * const shell_vars) {
	...
	for (size_t i = 1; i < args->size; i++) {
		char *const elem = args->data[i];

		if (regex_match("^[_0-9a-zA-Z!%,@]*$", elem).is_found) {
			char *maybe_alias = string_list_get_value(shell_vars->alias, elem);
			if (maybe_alias) {
				printf("%s=\'%s\'\n", elem, maybe_alias);
			} else {
				ft_dprintf(STDERR_FILENO, "42sh: alias: %s: not found\n", elem);
			}
		} else if (regex_match("^[_0-9a-zA-Z!%,@]+=", elem).is_found) {
			string_list_add_or_update(shell_vars->alias, elem);
		}
	}
