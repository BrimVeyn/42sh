...
/*   Updated: 2024/10/01 15:19:11 by bvan-pae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void hash_table_delete_entry(Entry **table, size_t index) {
	...
	while (current) {
		...
		gc_free(current->command, GC_SUBSHELL);
		gc_free(current->bin, GC_SUBSHELL);
		gc_free(current, GC_SUBSHELL);
		current = tmp;
	}
	table[index] = NULL;
}
...
Entry *entry_init(char *command, hash_mode mode) {
	Entry *self = gc_add(ft_calloc(1, sizeof(Entry)), GC_SUBSHELL);
	self->command = gc_add(ft_strdup(command), GC_SUBSHELL);
...
bool hash_table_add(Entry **table, char *entry, Vars *shell_vars, hash_mode mode) {
	...
	if (table[index] == NULL) {
		...
		Entry *current = table[index];
		if (hash_table_update(current, entry)) return true;
