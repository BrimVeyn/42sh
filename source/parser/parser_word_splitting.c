...
static TokenList *ft_token_split(const char *str, const char *ifs) {
	...
	for (uint16_t i = 0; i < str_len; i++) {
		Token *word = (Token *) gc(GC_ADD, token_empty_init(), GC_SUBSHELL);
		word->tag = T_WORD;
...
static TokenList *word_splitter(char *str, Vars *shell_vars) {
	...
	if (list->size == 0) {
		...
		word->tag = T_WORD;
		token_word_init(word);
		word->w_infix = (char *) gc(GC_ADD, ft_strdup(""), GC_SUBSHELL); //must be NULL
...
bool parser_word_split(TokenList *dest, Vars *shell_vars, char *prefix, char *infix, char *postfix, int index) {
	...
	first->w_infix = (char *) gc(GC_ADD, ft_strjoin(prefix, first->w_infix), GC_SUBSHELL);
	last->w_infix = (char *) gc(GC_ADD, ft_strjoin(last->w_infix, postfix), GC_SUBSHELL);
