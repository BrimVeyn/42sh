...
/*   Updated: 2024/09/19 09:51:09 by bvan-pae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
TokenList *lexer_lex_all(Lexer_p l) {
	...
	while (true) {
		Token *tmp = lexer_get_next_token(l, DEFAULT);
...
Token *lexer_get_next_token(Lexer_p l,type_mode mode) {
	...
	switch(token->tag) {
		...
		case T_REDIRECTION:
			...
			eat_whitespace(l);
			token->r_postfix = lexer_get_next_token(l, mode);
		...
		case T_WORD:
			...
			if (is_number(token->w_infix) && !is_whitespace(l->ch) && is_fdable_redirection(l)) {
				token->w_postfix = lexer_get_next_token(l, mode);
			} else {
				break;
			}
