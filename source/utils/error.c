...
/*   Updated: 2024/11/27 13:03:18 by bvan-pae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void pretty_error(char *raw_token) {
	...
	int lineno_len = ft_sprintf(buffer, "  %ld", lexer->pos.line) - message_len;
	ft_sprintf(buffer, "  |  ");
	char *line = get_line_x(raw_input, lexer->pos.line);
	int tab_count = 0;
	...
	ft_sprintf(buffer, "  |  ");
	for (size_t i = 0; i < lexer->pos.column - 1; i++) {
		if (tab_count) {
			ft_sprintf(buffer, "\t");
			tab_count -= 1;
		} else {
